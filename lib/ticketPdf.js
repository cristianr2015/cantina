...
const { formatRegionalMoney, localeForRegion, taxIdentifierEntries } = require('./regionalSettings');
...
function formatMoney(value, settings = {}) {
  return formatRegionalMoney(value, settings);
}
...
function ticketValueLabel(ticket, settings = {}) {
  return ticket?.ticket_type === 'cortesia'
    ? 'CORTESIA - SIN CARGO'
    : formatMoney(ticket?.price_paid, settings);
}
...
async function buildTicketPdf(tickets, settings = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    tickets.forEach((ticket, index) => {
      ...
      const details = [
        ...taxIdentifierEntries(settings).map(([identifier, value]) => `${identifier}: ${value}`),
        settings.address,
      ...
      doc.text(`Emitida: ${new Date(ticket.sold_at || Date.now()).toLocaleString(localeForRegion(settings.region_code))}`, 42, 173);
      ...
      doc.fillColor(ticket.ticket_type === 'cortesia' ? '#7C3AED' : '#111827')
        ...
        .text(ticketValueLabel(ticket, settings), 42, 210, { width: 230 });
